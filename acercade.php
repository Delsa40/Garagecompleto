<?php require('header.php') ?>
    <section>
        <h1>Acerca de</h1>
        <p>
            En Garagecompleto encontrarás información, juegos y novedades de autos, camionetas y motos.
            No importa si son deportivos, lentos o feos, en un garage completo hay de todo.
            Del mismo modo que se seleccionan las uvas para hacer un buen vino, en Garagecompleto 
            seleccionamos las mejores fotos, con sus respectivos créditos, para crear la mejor estética
            posible.<br>
            La pestaña Novedades cambia constantemente para que no te pierdas las noticias de la última semana. En 
            la pestaña Marcas elige una para seleccionar entre sus respectivos modelos, en donde cada cierto tiempo se 
            agregan más y más. También en la pestaña Marcas, si bajas hasta el fondo encontrarás las Rarezas, llena de 
            marcas y modelos que seguro no conocías. Cada modelo, ya sea de las marcas o rarezas, tendrá su ficha técnica 
            y datos de interes. Además hay como mínimo 2 fotos del auto(si estás usando una computadora de escritorio 
            pasa el cursor por encima de la primera foto para acceder a la segunda). En la pestaña Juegos no habrá un 
            Gran Turismo o Forza pero si habrán juegos entretenidos para pasar el rato y poner a prueba tus conocimientos.
        </p>
        <p>
            Quiero agredecer a todos los que apoyan la cuenta cada día. Conocí un montón de gente buena onda
            con la que cada vez aprendo más sobre esta linda pasión.
        </p>
    </section>
    <?php require('footer.php') ?>